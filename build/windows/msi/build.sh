...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="${APP_NAME:-VSCodium} - Insiders"
  PRODUCT_CODE="StarfishPulseInsiders"
  ...
  PRODUCT_NAME="${APP_NAME:-VSCodium}"
  PRODUCT_CODE="StarfishPulse"
...
CLEAN_VERSION="${CLEAN_VERSION%-insider}"
...
"${WIX}bin\\candle.exe" -arch "${PLATFORM}" vscodium.wxs "Files-${OUTPUT_BASE_FILENAME}.wxs" -ext WixUIExtension -ext WixUtilExtension -ext WixNetFxExtension -dManufacturerName="Starfish Logic Labs" -dAppCodeName="${PRODUCT_CODE}" -dAppName="${PRODUCT_NAME}" -dProductVersion="${CLEAN_VERSION}" -dProductId="${PRODUCT_ID}" -dBinaryDir="${BINARY_DIR}" -dIconDir="${ICON_DIR}" -dLicenseDir="${LICENSE_DIR}" -dSetupResourcesDir="${SETUP_RESOURCES_DIR}" -dCulture="${CULTURE}"
